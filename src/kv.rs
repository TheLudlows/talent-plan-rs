...
impl KvStore {
    ...
    fn compact(&mut self) -> Result<()> {
        ...
        for (key, pos) in self.index.iter() {
            ...
            if let Set { value, .. } = serde_json::from_reader(&mut reader)? {
                ...
                let off = new_writer.pos;
                serde_json::to_writer(&mut new_writer, &op)?;
                new_writer.flush()?;
                new_index.insert(key.clone(), Pos { id: self.cur_file_id, off, size: new_writer.pos - off });
            }
        }
    ...
    pub fn set(&mut self, key: String, value: String) -> Result<()> {
        ...
        self.writer.flush()?;
        if let Some(old) = self.index.insert(key, Pos { id: self.cur_file_id, off, size: self.writer.pos - off }) {
